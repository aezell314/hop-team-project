...
WITH 
	REFERAL_PROVIDER_CLASSIFICATIONS AS (
		SELECT
			CODE,
			CLASSIFICATION
		FROM NUCC
	...
	PRIMARY_TAXONOMY_CODES AS (
		...
		FROM NPPES
		WHERE ADDRESS_STATE_NAME IN ('TENNESSEE', 'TN')
		ORDER BY HEALTHCARE_PRIMARY_TAXONOMY_CODE DESC
	)
SELECT
	N.NPI,
	N.ORGANIZATION_NAME,
	...
	C.CLASSIFICATION
FROM NPPES N
	INNER JOIN PRIMARY_TAXONOMY_CODES T USING (NPI)
	INNER JOIN REFERAL_PROVIDER_CLASSIFICATIONS C ON T.HEALTHCARE_PRIMARY_TAXONOMY_CODE = C.CODE
